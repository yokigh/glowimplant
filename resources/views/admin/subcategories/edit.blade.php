...
<form action="{{ route('subcategories.update', ['subcategory' => $subcategory->id, 'lang' => app()->getLocale()]) }}" method="POST" enctype="multipart/form-data" class="row">
    @csrf
    @method('PUT')
        <div class="col-12">
        <div class="card">
            <div class="card-body">
                ...
                @foreach ($languages as $lang)
                    <div class="form-group mb-3 row">
                        ...
                        <div class="col-md-10">
                            <input class="form-control" type="text" name="name_{{ $lang }}" value="{{ $subcategory['name_' . $lang] }}">
                        </div>
                    </div>
                    ...
                    <div class="form-group mb-3 row">
                        ...
                        <div class="col-md-10">
                            <textarea class="form-control" id="desc_{{ $lang }}" name="description_{{ $lang }}">{{ $subcategory['description_' . $lang] }}</textarea>
                        </div>
                    </div>
                @endforeach
                ...
                @foreach ($languages as $lang)
                    <div class="form-group mb-3 row">
                        ...
                        <div class="col-md-10">
                            <textarea class="form-control" id="benefits_{{ $lang }}" name="benefits_{{ $lang }}">{{ $subcategory['benefits_' . $lang] }}</textarea>
                        </div>
                    </div>
                    <div class="form-group mb-3 row">
                        <label class="col-md-2 col-form-label">{{ __('pages.technical_info') }} ({{ strtoupper($lang) }})</label>
                        <div class="col-md-10">
                            <textarea class="form-control" id="technical_info_{{ $lang }}" name="technical_info_{{ $lang }}">{{ $subcategory['technical_info_' . $lang] }}</textarea>
                        </div>
                    </div>
                    <div class="form-group mb-3 row">
                        <label class="col-md-2 col-form-label">{{ __('pages.clinical_cases') }} ({{ strtoupper($lang) }})</label>
                        <div class="col-md-10">
                            <textarea class="form-control" id="clinical_cases_{{ $lang }}" name="clinical_cases_{{ $lang }}">{{ $subcategory['clinical_cases_' . $lang] }}</textarea>
                        </div>
                    </div>
                    <div class="form-group mb-3 row">
                        <label class="col-md-2 col-form-label">{{ __('pages.publish_articles') }} ({{ strtoupper($lang) }})</label>
                        <div class="col-md-10">
                            <textarea class="form-control" id="publish_articles_{{ $lang }}" name="publish_articles_{{ $lang }}">{{ $subcategory['publish_articles_' . $lang] }}</textarea>
                        </div>
                    </div>
                @endforeach
                ...
                <div class="form-group">
                    ...
                    <div id="drop-area" class="drop-area">
                        ...
                        <img  src="{{asset($subcategory->image)}}" alt="Image Preview" style="margin-top: 10px; max-width: 100%;">
                        @endif
                    </div>
                </div>
                ...
                <div class="form-group">
                    ...
                    @if ($subcategory->catalog)
                        <p id="catalog-name" class="text-muted">تم رفع الملف: <a href="{{ asset($subcategory->catalog) }}" target="_blank">عرض الكتالوج</a></p>
                    @endif
                </div>

                <button type="submit" class="btn btn-primary waves-effect waves-light">{{ __('pages.create_user') }}</button>
            </div>
        </div>
    </div>
</form>
