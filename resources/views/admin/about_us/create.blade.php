...
<form action="{{ route('about-us.store', ['lang' => app()->getLocale()]) }}" method="POST" enctype="multipart/form-data" class="row">
    @csrf
    <div class="col-12">
        <div class="card">
            <div class="card-body">
                <h4 class="header-title">{{ __('pages.Create About Us') }}</h4>
                @php
                ...
                <div class="form-group">
                    ...
                    <div id="drop-area" class="drop-area">
                        ...
                        <img id="image-preview" 
                            ...
                            alt="Image Preview" 
                            style="{{ isset($product) && $product->image ? 'display: block; margin-top: 10px; max-width: 100%;' : 'display: none;' }}">
                    </div>
                </div>


                <button type="submit" class="btn btn-primary waves-effect waves-light">{{ __('pages.create_aboutus') }}</button>
            </div>
        </div>
    </div>
</form>
...
<script src="{{asset('assets/libs/tinymce/
.min.js')}}"></script>
